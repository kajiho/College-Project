...
const Library = () => {
    ...
    const handleBorrowBook = (subjectId, bookId) => {
        const subject = bcaBooks.find(s => s.id === subjectId);
        const book = subject.books.find(b => b.id === bookId);
        
        // Add to borrowed books list with dates
    ...
    const handleReturnBook = (subjectId, bookId) => {
        ...
        setBorrowedBooks(prev =>
            prev.filter(book => !(book.id === bookId && book.subject === subject.subject))
        );

        // Show return alert
        alert(`📚 Thank you! "${book.name}" has been returned successfully.\n\nWe hope you enjoyed reading it!`);
    };
